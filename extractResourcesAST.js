  ...
  if (ts.isClassDeclaration(node) && node.name && node.name.text === 'EntityCommandService') {
    for (const member of node.members) {
      if (ts.isMethodDeclaration(member) && member.name) {
        ...
        if (resourceMethods.includes(methodName)) {
          ...
          const delegationBody = '{\n    return ResourceCommandHandler.' + methodName + '(' + params + ');\n  }';
...
if (!modifiedSourceCode.includes('import { ResourceCommandHandler }')) {
...
fs.writeFileSync('services/command/handlers/ResourceCommandHandlerBody.ts', handlerMethodsCode);
