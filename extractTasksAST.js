...
const taskMethods = [
  ...
  'moveTask', 'recycleTask', 'restoreTask', 'completeTasks', 
  'archiveTasks', 'recycleTasks', 'clearCompletedTasks', 
  'permanentlyDeleteTask', 'restoreTasks', 'reorderTasks'
];
  ...
  if (ts.isClassDeclaration(node) && node.name && node.name.text === 'EntityCommandService') {
    for (const member of node.members) {
      if (ts.isMethodDeclaration(member) && member.name) {
        ...
        if (taskMethods.includes(methodName)) {
          ...
          const delegationBody = '{\n    return TaskCommandHandler.' + methodName + '(' + params + ');\n  }';
...
if (!modifiedSourceCode.includes('TaskCommandHandler')) {
...
fs.writeFileSync('services/command/handlers/TaskCommandHandlerBody.ts', handlerMethodsCode);
