  ...
  if (ts.isClassDeclaration(node) && node.name && node.name.text === 'EntityCommandService') {
    for (const member of node.members) {
      if (ts.isMethodDeclaration(member) && member.name) {
        ...
        if (workspaceMethods.includes(methodName)) {
          ...
          const delegationBody = '{\n    return WorkspaceCommandHandler.' + methodName + '(' + params + ');\n  }';
...
if (!modifiedSourceCode.includes('WorkspaceCommandHandler')) {
...
fs.writeFileSync('services/command/handlers/WorkspaceCommandHandlerBody.ts', handlerMethodsCode);
