...
describe("Phase 0 event and reload contracts", () => {
  ...
  test.failing("requires TodayActions tasks_changed emission to carry a stable emitter id", () => {
    const source = fs.readFileSync(path.resolve(__dirname, "../../../today/hooks/useTodayActions.ts"), "utf8");
    expect(source).toMatch(/emitStateChange\(["']tasks_changed["'],\s*["'][^"']+["']\)/);
  });

  test.failing("requires Today dashboard to subscribe to mutation events while mounted", () => {
    const source = fs.readFileSync(path.resolve(__dirname, "../../../today/hooks/useTodayDashboard.ts"), "utf8");
