    ...
    for (const member of node.members) {
      if (ts.isMethodDeclaration(member) && member.name) {
        ...
        if (systemMethods.includes(methodName)) {
          ...
          const delegationBody = '{\n    return SystemCommandHandler.' + methodName + '(' + params + ');\n  }';
...
if (!modifiedSourceCode.includes('import { SystemCommandHandler }')) {
...
fs.writeFileSync('services/command/handlers/SystemCommandHandlerBody.ts', handlerMethodsCode);
