...
export function normalizeTask(rawTask: any, defaultWorkspaceId: string): Task {
  ...
  if (!reminder && (rawTask.alarmTime || rawTask.notificationIds)) {
    reminder = {
      ...
      notificationIds: rawTask.notificationIds || (rawTask.alarmId ? [rawTask.alarmId] : undefined),
    };
  }
  ...
  if (!recurrence && rawTask.recurrenceConfig) {
    ...
    let freq: RecurrenceRule["frequency"] = "daily";
    if (recurrence.type === "weekly" || recurrence.type === "weekdays") freq = "weekly";
  ...
  return {
    ...
    completedAt: rawTask.completedAt || (status === "completed" ? Date.now() : undefined),
    archivedAt: rawTask.archivedAt || (rawTask.archived ? Date.now() : undefined),
  };
}
...
export class TaskRepository {
  ...
  static async saveTasks(tasks: any[], workspaceId: string): Promise<void> {
    ...
    for (const task of tasks) {
      const targetWorkspaceId = workspaceId || task.workspaceId || DEFAULT_WORKSPACE_ID;
