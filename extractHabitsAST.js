  ...
  if (ts.isClassDeclaration(node) && node.name && node.name.text === 'EntityCommandService') {
    for (const member of node.members) {
      if (ts.isMethodDeclaration(member) && member.name) {
        ...
        if (habitMethods.includes(methodName)) {
          ...
          const delegationBody = '{\n    return HabitCommandHandler.' + methodName + '(' + params + ');\n  }';
...
if (!modifiedSourceCode.includes('import { HabitCommandHandler }')) {
...
fs.writeFileSync('services/command/handlers/HabitCommandHandlerBody.ts', handlerMethodsCode);
